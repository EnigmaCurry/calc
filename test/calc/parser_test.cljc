...
(deftest clean-phrase-normalizes-whitespace
  (testing "collapses spaces around slash between words"
    (is (th/deep== "2 meters/second" (parser/clean-phrase "2 meters / second")))
    (is (th/deep== "2 meters/second" (parser/clean-phrase "2 meters/   second")))
    (is (th/deep== "2 meters/second" (parser/clean-phrase "2 meters/ second"))))

  (testing "collapses spaces around slash between digits"
    (is (th/deep== "21349/234234" (parser/clean-phrase "21349 /234234")))
    (is (th/deep== "21349/234234" (parser/clean-phrase "21349 / 234234")))
    (is (th/deep== "21349/234234" (parser/clean-phrase "21349/234234"))))

  (testing "does not collapse slash between unit and number"
    (is (th/deep== "100 MB / 100 Mbps" (parser/clean-phrase "100 MB / 100 Mbps"))))

  (testing "spaces slash between letter and digit"
    (is (th/deep== "37 W / 12 v" (parser/clean-phrase "37 W /12v")))
    (is (th/deep== "37 W / 12 v" (parser/clean-phrase "37W/12v")))
    (is (th/deep== "W / 12" (parser/clean-phrase "W/12")))
    (is (th/deep== "12 / v" (parser/clean-phrase "12/v"))))

  (testing "spaces star between letter and digit"
    (is (th/deep== "12 V * 1 A" (parser/clean-phrase "12V*1A")))
    (is (th/deep== "12 V * 1 A" (parser/clean-phrase "12V * 1A")))
    (is (th/deep== "V * 1" (parser/clean-phrase "V*1")))
    (is (th/deep== "12 * v" (parser/clean-phrase "12*v"))))

  (testing "preserves star between letters for unit multiplication"
    (is (th/deep== "ft*ft" (parser/clean-phrase "ft*ft")))))
...
(deftest parses-simple-scalar-conversions
  (testing "basic '<number> <unit> in/to <unit>' phrases"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "5 miles to km"
      {:op :convert
       :quantity {:value 5 :unit :mi}
       :to :km}

      "3 gallons in liters"
      {:op :convert
       :quantity {:value 3 :unit :gal}
       :to :l}

      "100 pounds to kilograms"
      {:op :convert
       :quantity {:value 100 :unit :lb}
       :to :kg})))
...
(deftest parses-reversed-how-many-form
  (testing "'how many target units are in value source unit' reverses target/source correctly"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 3 :unit :ft}
       :to :in}

      "how many cups are in a gallon?"
      {:op :convert
       :quantity {:value 1 :unit :gal}
       :to :cup}
...
(deftest parses-temperature-conversions
  (testing "temperature still parses like scalar conversion; evaluator handles affine math"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "32 F in C"
      {:op :convert
       :quantity {:value 32 :unit :degF}
       :to :degC}

      "100 celsius to fahrenheit"
      {:op :convert
       :quantity {:value 100 :unit :degC}
       :to :degF}

      "273.15 K in C"
      {:op :convert
       :quantity {:value 273.15 :unit :K}
       :to :degC})))
...
(deftest parses-compound-rate-units
  (testing "compound units become exponent maps"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 60 :unit {:mi 1 :hr -1}}
       :to {:km 1 :hr -1}}

      "10 meters per second to mph"
      {:op :convert
       :quantity {:value 10 :unit {:m 1 :s -1}}
       :to {:mi 1 :hr -1}}

      "5 ft/s in m/s"
      {:op :convert
       :quantity {:value 5 :unit {:ft 1 :s -1}}
       :to {:m 1 :s -1}})))
...
(deftest parses-derived-named-units
  (testing "named derived units may appear beside structural compound units"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 1 :unit :N}
       :to {:kg 1 :m 1 :s -2}}

      "1 joule in newton meters"
      {:op :convert
       :quantity {:value 1 :unit :J}
       :to {:N 1 :m 1}}

      "1 watt in joules per second"
      {:op :convert
       :quantity {:value 1 :unit :W}
       :to {:J 1 :s -1}}

      "1 psi in pascals"
      {:op :convert
       :quantity {:value 1 :unit :psi}
       :to :Pa})))
...
(deftest parses-data-units
  (testing "data units preserve decimal/binary distinction"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 1024 :unit :B}
       :to :KB}

      "1 GiB in MiB"
      {:op :convert
       :quantity {:value 1 :unit :GiB}
       :to :MiB}

      "1 megabit in megabytes"
      {:op :convert
       :quantity {:value 1 :unit :Mb}
       :to :MB}

      "100 Mbps in MB/s"
      {:op :convert
       :quantity {:value 100 :unit {:Mb 1 :s -1}}
       :to {:MB 1 :s -1}})))
...
(deftest parses-mixed-quantities
  (testing "multiple compatible quantity terms become a vector"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity [{:value 5 :unit :ft}
                  {:value 11 :unit :in}]
       :to :cm}

      "6 lb 4 oz in grams"
      {:op :convert
       :quantity [{:value 6 :unit :lb}
                  {:value 4 :unit :oz}]
       :to :g})))
...
(deftest parser-reports-ambiguous-quantities
  (testing "both sides having quantities produces a specific error"
    (is (th/deep== {:error :ambiguous-quantities
...
(deftest parses-math-expressions
  (testing "parenthesised arithmetic in quantity"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "(10-3) miles in km"
      {:op :convert
       :quantity {:value 7 :unit :mi}
       :to :km}

      "(100/4) pounds in kg"
      {:op :convert
       :quantity {:value 25 :unit :lb}
       :to :kg}))

  (testing "nested parentheses"
    (is (th/deep== {:op :convert
            :quantity {:value 20 :unit :ft}
            :to :m}
  ...
  (testing "bare math with spaces between operators"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "100 - 37 pounds in kg"
      {:op :convert
       :quantity {:value 63 :unit :lb}
       :to :kg}

      "2 + 3 + 5 gallons in liters"
      {:op :convert
       :quantity {:value 10 :unit :gal}
       :to :l}))
  ...
  (testing "operator precedence"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 14 :unit :ft}
       :to :m}

      "(10-2*3) yards in feet"
      {:op :convert
       :quantity {:value 4 :unit :yd}
       :to :ft}))
  ...
  (testing "decimal arithmetic"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 4.0 :unit :kg}
       :to :lb}

      "(0.5*6) liters in gallons"
      {:op :convert
       :quantity {:value 3.0 :unit :l}
       :to :gal}))
  ...
  (testing "exponentiation with ^"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "2^3 feet in inches"
      {:op :convert
       :quantity {:value 8 :unit :ft}
       :to :in}

      "(2^3) meters in feet"
      {:op :convert
       :quantity {:value 8 :unit :m}
       :to :ft}

      "3^2 yards in feet"
      {:op :convert
       :quantity {:value 9 :unit :yd}
       :to :ft}

      "2 ^ 4 pounds in kg"
      {:op :convert
       :quantity {:value 16 :unit :lb}
       :to :kg}))
  ...
  (testing "^ is right-associative and higher precedence than *"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      "(2^3^2) grams in kg"
      {:op :convert
       :quantity {:value 512 :unit :g}
       :to :kg}

      ;; 2*3^2 = 2*9 = 18
      "(2*3^2) feet in yards"
      {:op :convert
       :quantity {:value 18 :unit :ft}
       :to :yd}

      ;; 1+2^3 = 1+8 = 9
      "(1+2^3) miles in km"
      {:op :convert
       :quantity {:value 9 :unit :mi}
       :to :km}))
...
(deftest parses-percentage-expressions
  (testing "'X is what percent of Y' form"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :percentage :type :what-percent :value 10 :total 100}

      "25 is what percent of 200?"
      {:op :percentage :type :what-percent :value 25 :total 200}

      "1 is what percentage of 3"
      {:op :percentage :type :what-percent :value 1 :total 3}))

  (testing "'what percent of Y is X' form"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))

      "what percent of 100 is 25"
      {:op :percentage :type :what-percent :value 25 :total 100}

      "what percentage of 200 is 50"
      {:op :percentage :type :what-percent :value 50 :total 200}))

  (testing "'X percent of Y' form"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))

      "15 percent of 50"
      {:op :percentage :type :percent-of :percent 15 :value 50}

      "50 percent of 200"
      {:op :percentage :type :percent-of :percent 50 :value 200}
...
(deftest parses-slash-unit-separator
  (testing "'/' works like 'per' in unit phrases"
    (are [phrase expected] (th/deep== expected (parser/parse-request phrase))
      ...
      {:op :convert
       :quantity {:value 25 :unit {:mi 1 :hr -1}}
       :to {:km 1 :hr -1}}

      "100 feet / second in mph"
      {:op :convert
       :quantity {:value 100 :unit {:ft 1 :s -1}}
       :to {:mi 1 :hr -1}})))
...
(deftest parses-quantity-arithmetic
  (testing "division of quantities: data / rate = time"
    (is (th/deep== {:op :convert
            :quantity {:qty-expr true
                       :terms [{:value 100 :unit :MB}
                               {:value 100 :unit {:Mb 1 :s -1}}]
                       :ops [:/]}
            :to :s}
  ...
  (testing "quantity division parses with compact spacing"
    (let [expected {:op :convert
                    :quantity {:qty-expr true
                               :terms [{:value 37 :unit :W}
                                       {:value 12 :unit :V}]
                               :ops [:/]}
                    :to :auto}]
      (is (th/deep== expected (parser/parse-request "37 W / 12v")))
      (is (th/deep== expected (parser/parse-request "37 W /12v")))
      (is (th/deep== expected (parser/parse-request "37W/12v")))
      (is (th/deep== expected (parser/parse-request "37W / 12V")))))

  (testing "quantity multiplication parses with compact spacing"
    (let [expected (parser/parse-request "12 V * 1 amp")]
      (is (th/deep== expected (parser/parse-request "12V*1amp")))
      (is (th/deep== expected (parser/parse-request "12V * 1amp")))
      (is (th/deep== expected (parser/parse-request "12V*1 amp")))))

  (testing "uppercase A is parsed as amps, not article"
    (let [req (parser/parse-request "15V*2.46A")]
      (is (th/deep== :A (:unit (second (:terms (:quantity req))))))))

  (testing "lowercase a remains an article"
    (let [req (parser/parse-request "half a gallon in liters")]
      (is (th/deep== :gal (:unit (:quantity req))))))
...
(deftest parses-scientific-notation
  (testing "integer scientific notation"
    (is (th/deep== 1.0E+10 (parser/parse-number-token "10E9")))
    (is (th/deep== 1E+3 (parser/parse-number-token "1e3"))))

  (testing "decimal scientific notation"
    (is (th/deep== 3.5E+6 (parser/parse-number-token "3.5e6")))
    (is (th/deep== 3.5E-12 (parser/parse-number-token "3.5e-12")))
    (is (th/deep== 1.5E+3 (parser/parse-number-token "1.5E+3"))))

  (testing "scientific notation in conversion phrases"
    (is (th/deep== {:op :convert
            :quantity {:value 1.0E+10 :unit :B}
            :to :GB}
           (parser/parse-request "10E9 bytes in GB")))

    (is (th/deep== {:op :convert
            :quantity {:value 3.5E+6 :unit :m}
            :to :km}
  ...
  (testing "scientific notation in math expressions"
    (is (th/deep== 2.0E+9 (parser/parse-math "1e9 + 1e9")))))
