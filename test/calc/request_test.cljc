...
(deftest evaluates-percentage-expressions
  (testing "what percent: 10 is what percent of 100 = 10%"
    (let [{:keys [result]} (th/evaluate "10 is what percent of 100" nil)]
      (is (= "10%" result))))

  (testing "what percent: 1 is what percent of 3"
    (let [{:keys [result]} (th/evaluate "1 is what percent of 3" nil)]
      (is (str/starts-with? result "33.3333"))))

  (testing "percent of: 15 percent of 50 = 7.5"
    (let [{:keys [result]} (th/evaluate "15 percent of 50" nil)]
      (is (= "7.5" result))))

  (testing "percent of: 50 percent of 200 = 100"
    (let [{:keys [result]} (th/evaluate "50 percent of 200" nil)]
      (is (= "100" result))))

  (testing "percent sign syntax: 15% of 50 = 7.5"
    (let [{:keys [result]} (th/evaluate "15% of 50" nil)]
      (is (= "7.5" result))))

  (testing "reversed form: what percent of 100 is 25 = 25%"
    (let [{:keys [result]} (th/evaluate "what percent of 100 is 25" nil)]
      (is (= "25%" result))))

  (testing "what is X percent of Y form"
    (let [{:keys [result]} (th/evaluate "what is 10 percent of 100" nil)]
      (is (= "10" result)))))
...
(deftest natural-language-formatting
  (testing "rounded to N decimals applies rounding"
    (let [{:keys [result target]} (th/evaluate "12 feet in yards rounded to 2 decimals" nil)]
      (is (= "4.00" result))
      (is (= "yd" target))))

  (testing "with N sig figs applies significant figures"
    (let [{:keys [result target]} (th/evaluate "5 miles in km with 3 sig figs" nil)]
      (is (= "8.05" result))
      (is (= "km" target))))

  (testing "CLI flags override parsed format"
    (let [{:keys [result]} (th/evaluate "12 feet in yards rounded to 2 decimals" {:round 4})]
      (is (= "4.0000" result))))

  (testing "no format clause leaves result unformatted"
    (let [{:keys [result]} (th/evaluate "12 feet in yards" nil)]
      (is (= "4" result))))

  (testing "as a fraction formats exact results"
    (let [{:keys [result target]} (th/evaluate "7 inches in feet as a fraction" nil)]
      (is (= "7/12" result))
      (is (= "ft" target))))

  (testing "as a fraction with integer result omits denominator"
    (let [{:keys [result target]} (th/evaluate "1 yard in feet as a fraction" nil)]
      (is (= "3" result))
      (is (= "ft" target))))

  (testing "as a fraction uses mixed number form"
    (let [{:keys [result target]} (th/evaluate "5 feet 11 inches in cm as a fraction" nil)]
      (is (= "180 17/50" result))
      (is (= "cm" target)))))

(deftest scalar-math-preserves-unit
  (testing "12 * 4 days = 48 days"
    (let [{:keys [result]} (th/evaluate "12 * 4 days" nil)]
      (is (= "48 days" result))))
  (testing "3 + 2 hours = 5 hours"
    (let [{:keys [result]} (th/evaluate "3 + 2 hours" nil)]
      (is (= "5 hours" result))))
  (testing "scalar math with explicit target still works"
    (let [{:keys [result from target]} (th/evaluate "12 * 4 days in seconds" nil)]
      (is (= "4147200" result))
      (is (= "s" target)))))

(deftest scalar-math-with-compound-units
  (testing "12 * 4 miles/hour = 48 mi/hr"
    (let [{:keys [result]} (th/evaluate "12 * 4 miles/hour" nil)]
      (is (= "48 mi/hr" result))))
  (testing "3 * 5 km/hr"
    (let [{:keys [result]} (th/evaluate "3 * 5 km/hr" nil)]
      (is (= "250 m/min" result))))
  (testing "2 + 3 feet/second"
    (let [{:keys [result]} (th/evaluate "2 + 3 feet/second" nil)]
      (is (= "91.44 m/min" result))))
  (testing "scalar math with compound unit and explicit target"
    (let [{:keys [result target]} (th/evaluate "12 * 4 miles/hour in km/hr" nil)]
      (is (= "77.248512" result))
      (is (= "km/hr" target)))))
...
(deftest compound-unit-syntax
  (testing "ft*ft parsed as square feet and converts to sq yards"
    (let [{:keys [result target]} (th/evaluate "1 ft*ft in sq yd" nil)]
      (is (some? result))
      (is (= "yd^2" target))))

  (testing "60 mph in ft/s"
    (let [{:keys [result target]} (th/evaluate "60 mph in ft/s" nil)]
      (is (= "88" result))
      (is (= "ft/s" target))))

  (testing "m*m parsed as square meters"
    (let [{:keys [result target]} (th/evaluate "9 m*m in sq ft" nil)]
      (is (some? result))
      (is (= "ft^2" target))))

  (testing "sq ft as alias for square feet"
    (let [{:keys [result target]} (th/evaluate "1 sq ft in sq m" nil)]
      (is (some? result))
      (is (= "m^2" target))))

  (testing "squared yard (prefix form)"
    (let [{:keys [result target]} (th/evaluate "1 ft*ft in squared yard" nil)]
      (is (some? result))
      (is (= "yd^2" target))))

  (testing "yard square (suffix form)"
    (let [{:keys [result target]} (th/evaluate "1 ft*ft in yard square" nil)]
      (is (some? result))
      (is (= "yd^2" target))))

  (testing "yard squared (suffix form)"
    (let [{:keys [result target]} (th/evaluate "1 ft*ft in yard squared" nil)]
      (is (some? result))
      (is (= "yd^2" target))))

  (testing "cu ft as alias for cubic feet"
    (let [{:keys [result target]} (th/evaluate "1 cu ft in cu m" nil)]
      (is (some? result))
      (is (= "m^3" target))))

  (testing "foot cubed (suffix form)"
    (let [{:keys [result target]} (th/evaluate "1 foot cubed in meter cubed" nil)]
      (is (some? result))
      (is (= "m^3" target))))

  (testing "sqft shorthand"
    (let [{:keys [result]} (th/evaluate "12 sqft" nil)]
      (is (= "12 ft^2" result))))

  (testing "sqft to sqm conversion"
    (let [{:keys [result target]} (th/evaluate "100 sqft in sqm" nil)]
      (is (some? result))
      (is (= "m^2" target))))

  (testing "cuft shorthand"
    (let [{:keys [result]} (th/evaluate "1 cuft" nil)]
      (is (= "1 ft^3" result)))))
